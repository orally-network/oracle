...
export const useApolloDeposit = ({ closeModal, chain }: UseApolloDepositParams) => {
  const { sendTransactionWithWait, isPending: isTransferring } = useSendTransactionWithWait('Transferring funds to Apollo');
  const { mutate: deposit, isPending: isDepositing } = useDeposit();

  const apolloDeposit = useCallback(async (chainId: number, _: AllowedToken, amount: number) => {
    if (!chain) return;
