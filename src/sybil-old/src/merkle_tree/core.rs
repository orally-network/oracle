...
pub fn sibling_index(i: i32) -> usize {
    if i > 0 {
...
pub fn check_tree_node(tree: &[Bytes], i: usize) -> () {
...
pub fn check_internal_node(tree: &[Bytes], i: usize) -> () {
...
pub fn get_proof(tree: Vec<Bytes>, mut i: usize) -> Vec<Bytes> {
    ...
    while i > 0 {
        let sibling_i = sibling_index(i.try_into().unwrap());
...
pub fn get_multi_proof(tree: Vec<Bytes>, indices: &mut [usize]) -> MultiProof<Bytes, Bytes> {
    ...
    while !stack.is_empty() && stack[0] > 0 {
        ...
        let s = sibling_index(j.try_into().unwrap());
